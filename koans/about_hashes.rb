...
class AboutHashes < Neo::Koan
  def test_creating_hashes
    empty_hash = Hash.new #creates a new hash - a dictionary-like collection of key value pairs
    assert_equal Hash, empty_hash.class
    assert_equal({}, empty_hash)
    assert_equal 0, empty_hash.size # size returns the number of key/value pairs
  end
  ...
  def test_accessing_hashes_with_fetch
    hash = { :one => "uno" }
    assert_equal "uno", hash.fetch(:one) # fetch returns the value of a given key
    assert_raise(KeyError) do # if a key is not found
  ...
  def test_changing_hashes
    ...
    # seems we created this "expected" variable to define what we are literally expecting our hash to be and comparing it to the original hash
    # since we changed the value of the :one key to "eins"

    # From Stack overflow
    # It's because you can't use something like this: assert_equal { :one => "eins", :two => "dos" }, hash
  ...
  def test_hash_keys
    ...
    assert_equal Array, hash.keys.class # .keys returns a new array of the keys from the hash
  end
  ...
  def test_default_value_is_the_same_object
    ...
    hash[:one] << "uno" # << means add to the end of an array
    hash[:two] << "dos"
